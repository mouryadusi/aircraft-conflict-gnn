...
# --- Build pairs_df ---
pair_records = []
for t, group in region_final.groupby("time"):    aircraft = group.set_index("icao24")
...
for (t, day), _ in region_final.groupby(["time", "source_day"]):
    ...
    x = torch.tensor(
        nodes[node_feature_cols].fillna(0).values,
        dtype=torch.float,
    )

    edge_attr = torch.tensor(
        edges[edge_feature_cols].fillna(0).values,
        dtype=torch.float,
    )
...
print("\nSaved processed_data/graphs_large.pt")
