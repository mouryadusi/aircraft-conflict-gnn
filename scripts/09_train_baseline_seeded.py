...
print("Loaded baseline graphs:", len(graphs))
...
model = ConflictGAT(node_dim=6, edge_dim=5).to(device)
...
print("\nBest validation F1 (Baseline, seeded):", best_f1)

if best_state is not None:
    model.load_state_dict(best_state)
    torch.save(best_state, "models/conflict_gat_baseline_seeded.pth")

y_true, y_pred, y_prob = evaluate(model, test_loader, device)
results = {
    "model": "GAT", "feature_set": "trajectory_only",
    "n_node_features": 6, "n_edge_features": 5,
...
print("\nBaseline (seeded) test results:", results)

with open("processed_data/gat_baseline_seeded_results.json", "w") as f:
    json.dump(results, f, indent=2)

print("DONE: 09_train_baseline_seeded.py")
